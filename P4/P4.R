#Práctica 4: diagramas de Voronoi. Código base en Ref. [1].
#Se crea la zona
largos=data.frame()
...
propaga <- function(replica) {
  ...
  euclidiana=function(largo){return(sqrt(sum((i[1]-xg)-(i[2]-yg)**2)))}
  return(largo)
     }
largos=rbind(largos,foreach(r = 1:10, .combine=c) %dopar% propaga(r))
  }
}
suppressMessages(library(doParallel))
registerDoParallel(makeCluster(detectCores() - 1))
...
png("p4_semillas.png")
lbls = c("40", "60", "80", "100")
boxplot(n40, n60, n80, n100, col=rainbow(4), names = lbls, ylab="Distancia euclidiana", xlab="Tamaño de zona (n x n)", ylim = c(1, 200))
graphics.off()
